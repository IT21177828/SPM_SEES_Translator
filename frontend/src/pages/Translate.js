...
export default function Translate() {
      ...
      const response = await axios
        ...
        .then((res) => {
          if (res.data.user) {
            ...
            setIsLogedIn(true);
            isMembershilActive(res.data.user.email);
          } else {
            setIsLogedIn(false);
          }
        })
  ...
  const isMembershilActive = async (email) => {
    try {
      console.log(email);
      await axios
  ...
  const handleFeedbackSubmit = async () => {
    try {
      if (feedback.feedbackText.length > 50) {
        ...
        alertDiv.className =
          "bg-blue-100 border-t border-b border-blue-500 text-blue-700 px-4 py-3 text-center";
