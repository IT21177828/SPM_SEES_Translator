...
export const Checkout = () => {
    ...
    const [formData, setFormData] = useState({
      ...
      clearError();
          ...
          const response = await axios.post('http://localhost:5050/checkout/saveCheckout', formData);
      ...
      return (
        <div className='flex'>
            <div className='bg-white text-center p-14 mr-2 rounded-lg '>
                <h1 className='font-bold text-lg'style={{ fontSize: '2rem' }}>Summery</h1>
                <div className='mt-44'>
        ...
        <form onSubmit={handleSubmit}>
            {/* Display error message */}
                ...
                {error && <div className='text-red-500 text-center bg-red-100 p-2 rounded-lg'>{error}</div>}
          ...
          <button type="submit" className='bg-green-500 text-white font-bold py-1 px-2 rounded-lg h-10 mt-4 w-full'>CheckOut</button>
        </form>
        </div>
        </div>
      );
